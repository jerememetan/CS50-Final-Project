...
def index():
    ...
    today_bookings = db.execute("SELECT * FROM bookings WHERE users_id =? AND date =? ORDER BY date", session["user_id"], today.strftime("%Y-%m-%d"))
        ...
        for bookings in today_bookings:
            ...
            if len(possibletags) > 0:
                for tags in possibletags:
                    ...
                    if not bookings["type"]:
                        bookings["type"] = tag_name[0]["tag_name"]
                    # if the tag name is in not in type
                    elif bookings["type"] and tag_name[0]["tag_name"] not in bookings["type"]:
                        bookings["type"] += " , " + tag_name[0]["tag_name"]
                # sets the type into the database table
            db.execute("UPDATE bookings SET type =? WHERE id =?", bookings["type"], bookings["id"])
            formatbooking(bookings)
    # list upcoming bookings
    upcoming_bookings = db.execute("SELECT * FROM bookings WHERE users_id =? AND date >? ORDER BY date", session["user_id"], today.strftime("%Y-%m-%d"))
    if len(upcoming_bookings) == 0:
        ...
        for bookings in upcoming_bookings:
            ...
            if len(possibletags) > 0:
                for tags in possibletags:
                    tag_name = db.execute("SELECT tag_name FROM usertags WHERE id = ?", tags["usertags_id"])
                    # check if type is blank, if it is blank,
                    if not bookings["type"]:
                        bookings["type"] = tag_name[0]["tag_name"]
                    # if the tag name is in not in type
                    elif bookings["type"] and tag_name[0]["tag_name"] not in bookings["type"]:
    ...
    previous_bookings = db.execute("SELECT * FROM bookings WHERE users_id =? AND date <? ORDER BY date", session["user_id"], today.strftime("%Y-%m-%d"))
...
def book():
    ...
    if request.method == "POST":
        ...
        selected_tag = []
        booking = db.execute("SELECT * FROM bookings WHERE id = ?", booking_id)
